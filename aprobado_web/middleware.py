...
class RetiredLegacySurfaceMiddleware:
    ...
    def __call__(self, request):
        host = self._normalizar_host(request.get_host())
        disabled_hosts = {
            self._normalizar_host(value)
            for value in getattr(settings, 'LEGACY_DISABLED_HOSTS', ())
            if value
        }
        if getattr(settings, 'DEBUG', False):
            disabled_hosts.update(
                {
        ...
        disabled_media_prefixes = (
            '/media/contractors/',
            '/media/pagares/',
            '/media/pagares_firmados/',
        )
        if request.path.lower().startswith(disabled_media_prefixes):
            return HttpResponseNotFound()
...
class SubdomainRoutingMiddleware:
    """Enrutador historico conservado, pero retirado de MIDDLEWARE."""
    ...
    def __call__(self, request):
        ...
        if self._es_host_contratistas(host, contractors_host):
            ...
            redirect_host = self._redirect_host_for_path(
                request.path,
                primary_host,
                emprender_host,
                market_host,
    ...
    def _es_host_contratistas(host, contractors_host):
        ...
        return bool(
            ...
            and host == 'contratistas.localhost'
        )
    ...
    def _redirect(request, target_host):
        ...
        return HttpResponsePermanentRedirect(
            f'{scheme}://{target_host}{request.path}{suffix}'
        )
